...
public abstract class TurtleCommand extends UnaryNode {
	...
	/**
	 * Super class that is extended by all turtle command classes. A concise and
	 * abstract way of extending functionality for turtle commands who need
	 ...
	 * set the workspace
	 * 
	 * @param SLogoWorkspacesw - current workspace
	 */
	 ...
	 * 
	 * @param SLogoTurtleFactory
	 *            turtleFactory - current turtleFactory
	 */
	...
	/**
	 * closed way of allowing the super class to handle interacting with the
	 * specific turtle factory instance in order to create and return a turtle
	 * with a specific index
	 * 
	 * @param int
	 *            turtleIndexToGrab - index of turtle that we want
	 * @return SLogoCharacter
	 */
	protected SLogoCharacter grabTurtle(int turtleIndexToGrab) throws SLogoException {
		return turtleFactory.createTurtle(turtleFactory.getDefaultX(), turtleFactory.getDefaultX(), turtleIndexToGrab,
				turtleFactory.getDefaultDirection(), turtleFactory.getDefaultHidden(), turtleFactory.getDefaultShape());
	}
	 ...
	 * stamp
	 * 
	 * @param SLogoCharacterState
	 *            state - the turtle state that we want to stamp
	 * @return SLogoCharacter
	 */
	protected SLogoCharacter createStampTurtle(SLogoCharacterState state) throws SLogoException {
		return turtleFactory.createTurtle((int) state.getXCoor(), (int) state.getYCoor(), STAMP_ID,
				state.getDirection(), state.getHidden(), state.getShapeIndex());
	}
